...
// Modified to support Exchange 2000
#define EXCHANGE_RE		5

//$--HrBypassDsProxy------------------------------------------------------------
//  Set a registry key to bypass DS proxy
//  added 
// -----------------------------------------------------------------------------
HRESULT HrBypassDsProxy(				// RETURNS: return code
	IN	BOOL fBypass,					// bypass DS proxy for next calls if true
	OUT BOOL* pfKeyAdded);				// was key added ?
...
//$--HrGetExchangeStatusEx2000--------------------------------------------------
//  Get the current state of the Exchange server on a given machine.
//	Added to request the status of Exchange 2000
// -----------------------------------------------------------------------------
HRESULT HrGetExchangeStatusEx2000(         // RETURNS: return code
    IN LPSTR lpszMachineName,           // machine name
    OUT DWORD *lpdwService,             // service
    OUT DWORD *lpdwCurrentState);       // current state
