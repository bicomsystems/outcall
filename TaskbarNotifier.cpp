...
int CTaskbarNotifier::Create(CWnd *pWndParent)
{
	...
	ret = CreateEx(0,strWndClass,NULL,WS_POPUP,0,0,0,0,pWndParent->m_hWnd,NULL);
	...
	switch (m_nAnimStatus)
	{
		case IDT_HIDDEN:
			...
			if (bTaskbarOnRight)
			{
				...
				m_nStartPosX=(rcDesktop.right-nDisplayedAlready*m_nSkinWidth);
			...
			else if (bTaskbarOnLeft)
			{
				...
				m_nStartPosX=(rcDesktop.left-m_nSkinWidth+nDisplayedAlready*m_nSkinWidth);
			...
			else if (bTaskBarOnTop)
			{
				...
				m_nStartPosY=(rcDesktop.top-m_nSkinHeight+nDisplayedAlready*m_nSkinHeight);
			...
			else //if (bTaskbarOnBottom)
			{
				...
				m_nStartPosY=(rcDesktop.bottom-nDisplayedAlready*m_nSkinHeight);
		...
		case IDT_DISAPPEARING:
			...
			SetWindowPos(&this->wndTopMost,m_nCurrentPosX,m_nCurrentPosY,m_nSkinWidth,m_nSkinHeight,SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_NOACTIVATE);
			RedrawWindow();
			break;
	}
}
...
void CTaskbarNotifier::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (::theApp.GetProfileInt("Settings", "OutlookFeatures", 1)==1)
		m_pWndParent->PostMessage(WM_TASKBARNOTIFIERCLICKED,(WPARAM)nDisplayedAlready,(LPARAM)0);
...
void CTaskbarNotifier::OnPaint()
{
	...
	dc.DrawText(CString(szBuffer),-1,m_rcText,DT_CENTER | DT_VCENTER | DT_WORDBREAK | DT_END_ELLIPSIS);
